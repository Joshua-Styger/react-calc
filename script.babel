...
const formatNumberString = (numString, maxLength = math.config.precision) => {
  ...
  if (numString.length <= maxLength) {
    ...
    if (numString.includes("e")) {
      ...
      return (
        numberParts[0].slice(0, maxLength - numberParts[1].length - 1) +
        "E" +
        numberParts[1]
      );
    } else {
...
const solveEquation = (expression) => {
  ...
  return math.format(result, { upperExp: 10 });
};
...
function createButtons(buttonHandler) {
  return buttonData.map((x) => (
    <button
      id={x.id}
      key={x.id}
      className={
        (x.span == 2 ? "doubleButton" : "singleButton") + " " + x.style
      }
      value={x.display}
...
const LCD_Display = (props) => {
  return (
    <div className="lcd-display" id={"display"}>
...
const Formula = (props) => {
  return <div className="formula"><p>{props.formula}</p></div>;
};

function Decorations() {
  return (
    <div className="decals">
      <h1 className="logo">SquidgeTech</h1>
      <div className="solar-charger"></div>
      <div className="grid">
        <h1 className="model">GS-C01</h1>
        <p className="detail">REACT POWER</p>
      </div>
    </div>
  );
}
  ...
  handleButtonPress(actionType, data) {
    ...
    switch (actionType) {
      case "number-entry":
        ...
        if (data == "0" && currentEntry == "0") {
          console.log("Cannot add leading zero");
        } else if (currentEntry.length < MAX_ENTRY_LENGTH) {
          newStateObject.entry =
            (currentEntry == "0" ? "" : currentEntry) + data;
          console.log(
            "Added input to entry",
            currentEntry,
            data,
            newStateObject.entry
          );
      ...
      case "op-entry":
        ...
        if (lastAction == "op-entry")
          newStateObject.formula = [...currentFormula.slice(0, -1), data];
        else {
          newStateObject.formula = [
            ...(lastAction == "solve" ? [] : currentFormula),
            this.state.entry,
            data,
          ];
        }
      ...
      default:
        console.log("No case present for actionType: " + actionType);
        return;
    }
    newStateObject.lastAction = actionType;
    this.setState(
      newStateObject,
      console.log("State after " + actionType, data, newStateObject, this.state)
    );
  }
    ...
    return (
      <div className="calculator-frame">
        <div className="calculator-container">
          ...
          <LCD_Display entry={formatNumberString(this.state.entry, MAX_ENTRY_LENGTH + (this.state.entry[0] == "-" ? 1 : 0))} formula={this.state.formula.map((x, index) => (index % 2 == 0 ? formatNumberString(x, MAX_ENTRY_LENGTH) : x)).join(" ")} />
          <ButtonArea buttonHandler={this.handleButtonPress} />
        </div>
        <p className='about'>Designed and coded by <span className='purple-text'>Joshua Styger</span></p>
      </div>
    );
  }
}
